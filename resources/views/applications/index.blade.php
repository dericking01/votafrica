        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <form method="GET" class="flex items-center gap-2">
                <div class="relative">
                    ...
                    <input name="search" value="{{ $search }}" placeholder="Search applications..."
                ...
                @if($search)
                    <a href="{{ route('applications.index') }}" class="px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">Clear</a>
                @endif
            </form>
        </div>
                ...
                <tbody>
                    @forelse($applications as $app)
                        <tr class="hover:bg-slate-50 transition-colors" style="border-bottom:1px solid #f8fafc;">
                            <td class="px-5 py-4">
                                <a href="{{ route('applications.show', $app) }}" class="font-semibold text-slate-900 hover:text-red-500 transition-colors text-sm">{{ $app->organization_name }}</a>
                            </td>
